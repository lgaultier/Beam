...
\title{Inversion des images à haute-résolution \hspace{4cm} pour le contrôle de la dynamique à plus large échelle \hspace{4cm} provenant de l'altimétrie}
...
\only<5>{ \node[fill=mycyan, text width=11.2cm] at (6.7,3) {\ding{226}Sous-échantillonnage de l'altimétrie : utilisation des données biogéochimiques et SST};
          \node[fill=mycyan, text width=11.2cm] at (6.7,2) {\ding{226}Projets SWOT : satellites altimétriques haute-résolution, nécessité de gérer un grand nombre de données};}
  \end{tikzpicture}
...
\begin{frame}
  ...
  \note{
  Je vais d'abord présenter la philosophie générale de cette étude, je détaillerai ensuite quelques idées clefs de la méthode. J'illustrerai cette méthode sur un exemple. }
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%10%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Correction du champ de vitesses à l'aide de la \alt<2>{Chl}{SST}}
  \begin{tabular}{cccc}
    %\hline
...
%11%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Résultats : Trajectoires Lagrangiennes \alt<2>{(traceur : Chl)}{(traceur : SST)}}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}%[!htbp]
  ...
  \begin{itemize}
    \item Les trajectoires des six particules sont représentés sur un champ de \alt<2>{Chlorophylle (mg.m$^-3$)}{température de surface (SST, $^o$C)} 
...
%    \item \only<2>{Velocity does not cross frontal structure anymore}
  ...
  \end{block}
  \note{
Sur ces deux figures sont tracés les trajectoires lagrangiennes de six particules. 
Les trajectoires lagrangiennes issues des vitesses corrigées sont en bien meilleur adéquation avec les traceurs que les trajectoires lagrangiennes issues des vitesses aviso. 
On peut toutefois noter que les trajectoires lagrangiennes diffèrent selon le choix du traceur utilisé pour effectuer la correction. 
} 
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
