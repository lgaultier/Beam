...
On one hand, the dynamic of the ocean is currently observed from space by altimetric satellites. 
...
\begin{frame}
 ...
 \begin{tikzpicture}
   ...
   \node[pname, text width=12cm] (J) at (9,4.00) {COST FUNCTION: $J(\vec{u})=\|I_{tracer}- I_{FSLE}(\vec{u})\| + background\ term$};
   \draw[fleche] (IMTRA)--(J);
   \draw[fleche] (IMDYN)--(J);
  \end{tikzpicture} \\
...
We have observation of the dynamic and of the tracer and we build the correspondig images 
