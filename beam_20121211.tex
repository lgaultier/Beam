...
\usetheme{Darmstadt} %{Frankfurt} %Darmstadt}
...
%  \end{center}
  \begin{center}
    \includegraphics[height=1.5cm]{./pict/logo/logo_meom.png}
    \hspace{0.5cm}
    \includegraphics[height=1.5cm]{./pict/logo/logo_lgge.jpeg}
    \hspace{0.5cm}
...
On s'intéresse dans ce travail à la dynamique méso et sous-mésoechelle (entre 1 et 100km). 
...
%    \node[color=green, text width=4cm, text centered] (TRA) at (0,2.7) {Sub-mesoscale tracer image};
  ...
  \visible<2->{
  \begin{block}{Approche similiare à une assimilation de donnée}
...
\section[Methode]{Une approche d'assimilation de données}
\begin{comment}
%6%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
%   \frametitle{L'inversion d'images traceur}
 ...
 \begin{tikzpicture}
    ...
    \node[pname,text width=4cm] (OBSSSH) at (13,7.98) {SSH provenant de l'Altimetrie};
  ...
  \begin{block}{}
    $\bullet$ Explorer le sous-espace d'erreur pour trouver la vitesse qui minimize la fonction coût. 
    L'espace des vitesses perturbées est contruit grâce à l'analyse EOF sur un ensemble de vitesse:
...
%  \end{figure}
  \begin{itemize}
    \item \textbf{Période temporelle}: from 1998 to June 2009, 595 velocity maps
    \item \textbf{Vitesse}: AVISO, Altimetric data
    \item \textbf{Résolution}: $1/3^o$, grid points : 13*17
    \item \textbf{Résolution FSLE}: $1/50^o$, grid points : 99*130
  \end{itemize}
%  \vsapce{1cm}
  \begin{itemize}
...
%\end{frame}
	...
	\section[Limites de la méthode]{Inversion sur un modèle}

\subsection{Le modèle de la mer des Salomons}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\begin{minipage}{0.49\textwidth}
\includegraphics[width=4.5cm]{./pict/hist_1_SOSMOD36-GND100L_19940909_votemper_fsle.png}\\
\legende{Angle entre le FSLE et le traceur, histogramme des angles}
\end{minipage}
\begin{minipage}{0.49\textwidth}
\includegraphics[width=4.5cm]{./pict/Interquantile.png}\\
        \legende{Premier quantile et Interquantile de la distribution spatiale des angles entre le FSLE et le traceur} 
\end{minipage}
\begin{block}{}
Dépendance temporelle de la similarité traceur-FSLE\\
Due à la MLD? au vent? 
\end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Le modèle a une grosse variabilité : difficile de construire un sous-espace d'erreur conhérent.\\
Dans l'étude suivante le sous-espace d'erreur est idéal. \\ 
\ding{226} Les 50 états du système précédant la date choisie pour l'inversion. 
\end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}
  ...
  \begin{block}{Assimiler les structures plutôt que la donnée point par point}
    \begin{itemize}
      ...
      \item Pour assimiler SWOT dans des modèles à résolution plus faible
    \end{itemize}
  \end{block}
...
\begin{frame}
  ...
  \begin{block}{}
   \textbf{Aujourd'hui, on peut inverser des données traceurs pour compléter les observations altimétriques.}
  \end{block}
  \begin{block}{}
  \begin{itemize}
    ...
    \item Les limites de l'inversion sont étudiées grâce à un modèle haute-résolution réaliste de la mer des Salomons
  \end{itemize}
  \end{block}
