...
%\frametitle{Strategy}
  ...
  \begin{enumerate}
    ...
    \item Model data: Estimate the veolicity and assess the error of the estimate.
    \begin{itemize}
      ...
      \item Realistic model: High Resolution realistic model of the Solomon Sea. Inversion of the SST and the SSS: Assess the required conditions to apply the method
    \end{itemize}
  \end{enumerate}
  \end{block}
\end{frame}
...
%\alt<2>{\frametitle{Study of the cost function: Full inversion}}
 ...
 \begin{block}{}
  ...
  \end{block}
\note{not aunique solution, a sample of likely solution}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
%Inversion with SST alone, Chl alone 
  ...
  \begin{minipage}{0.3\textwidth}
  \begin{center}
    \alt<2>{\includegraphics[width=1\linewidth]{./pict_nemo/image.png}}
...
%Inversion with SST alone, Chl alone 
  ...
  \begin{minipage}{0.6\textwidth}
  \begin{block}{}
    $\simeq$ 45\% of the error on the background velocity is corrected usi
ng the SST as a tracer. \\
  \end{block}
  \end{minipage}
\note{Here are the results of the inversion of the Chlorophyll or the SST image.
We can see that our methods enable us to correct the perturbation in the right direction.
...
%Salomon model
  \begin{tikzpicture}
    \node[anchor=west,inner sep=0] (pict) at (-6,0){\movie[externalviewer]{\includegraphics[width=6.5cm]{./pict_nemo/1_SOSMOD36-GND100L_1222_big.png}}{./pict_nemo/RV-SST_design_video_G1N_y93-00_lev1.avi}};
...
%    \node[anchor=west,inner sep=0] (AVISO) at (-6,0){\includegraphics[width=6.5cm]{}}; % pict model vel Solomon
...
%les images structures pour controler les structures  plus fines. 
  ...
  At small scales, using tracer images to control complex structures. \\
  \end{block}
\end{frame}
...
\frametitle{SWOT prospects}
 ...
 \end{block}
  ...
  \begin{itemize}
    ...
    \item Enhencement of the signal structure% le signal par rapport à de l'assimilation point par point %%%%%
